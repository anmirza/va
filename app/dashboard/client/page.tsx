...
import {
  Building2, Search, Phone, AlertTriangle, CreditCard,
  ChevronRight, MapPin, Globe, Users, ExternalLink, Zap,
} from 'lucide-react'
...
function ClientDashboardContent() {
  ...
  const remaining = company ? (company.tokens ?? 0) - (company.tokensUsed ?? 0) : 0
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-10 max-w-5xl mx-auto w-full">
        ...
        {creditsExhausted && (
          <div className="flex items-center gap-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl px-6 py-4 mb-8">
            ...
            <div className="flex-1">
              <p className="text-amber-300 font-semibold text-sm">Your programme credits have been used up.</p>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 mb-8">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">Agency Search Credits</p>
                  <p className="text-xs text-white/40">Each search uses 1 credit from your package</p>
                </div>
              </div>
              <div className="text-right">
                <p className={`text-2xl font-bold ${remaining <= 0 ? 'text-red-400' : remaining <= 2 ? 'text-amber-400' : 'text-white'}`}>
                  {remaining}
                </p>
                <p className="text-xs text-white/40">of {company.tokens} remaining</p>
              </div>
            </div>
            {/* Progress bar */}
            <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: company.tokens > 0 ? `${(remaining / company.tokens) * 100}%` : '0%' }}
              />
            </div>
            ...
            <div className="flex flex-wrap gap-4 mt-5 pt-5 border-t border-white/[0.06] text-xs text-white/40">
              {company.holdingCompany && (
                <span className="flex items-center gap-1.5"><Building2 className="w-3 h-3" /> {company.holdingCompany}</span>
              )}
              {company.region && (
                <span className="flex items-center gap-1.5"><MapPin className="w-3 h-3" /> {company.region}</span>
              )}
              {company.country && (
                <span className="flex items-center gap-1.5"><Globe className="w-3 h-3" /> {company.country}</span>
              )}
            </div>
          </div>
        )}
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 mb-8">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <p className="text-sm font-semibold text-white">Agency Search & Selection</p>
              <p className="text-xs text-white/40">Search by name, country or city. 1 credit per search.</p>
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-white text-sm group-hover:text-[#0763d8] transition-colors">{agency.name}</p>
        ...
        {!company && !loading && (
          <div className="text-center py-20 text-white/30">
            ...
            <p className="text-sm mt-2">Please contact VA Consulting to complete your account setup.</p>
            <a
              href="mailto:info@va-consulting.com"
