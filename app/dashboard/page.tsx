  ...
  return (
    <div className="min-h-screen bg-[#02030E] flex flex-col">
      ...
      <main className="flex-1">
        {/* Page header */}
        <div className="bg-gradient-to-r from-[#0a0c18] via-[#111328] to-[#0a0c18] px-4 sm:px-6 lg:px-8 py-10 border-b border-white/[0.06]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    return (
                      <Link key={link.href} href={link.href} className="group glass-card p-5 flex items-start gap-4 hover:border-[#4fc487]/30 transition-all">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-white group-hover:text-[#4fc487] transition-colors">{link.label}</p>
                          <p className="text-xs text-white/30 mt-0.5">{link.desc}</p>
                        </div>
                        <ChevronRight className="w-4 h-4 text-white/10 shrink-0 self-center group-hover:text-[#4fc487] transition-colors" />
                      </Link>
                    )
                  })}
                  <Link href="/signup/agency" className="group glass-card p-5 flex items-start gap-4 hover:border-[#4fc487]/30 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-white group-hover:text-[#4fc487] transition-colors">Register Agency</p>
                      <p className="text-xs text-white/30 mt-0.5">Add your agency to the directory</p>
                    </div>
                    <ChevronRight className="w-4 h-4 text-white/10 shrink-0 self-center group-hover:text-[#4fc487] transition-colors" />
                  </Link>
                  <Link href="/creative-library" className="group glass-card p-5 flex items-start gap-4 hover:border-[#4fc487]/30 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-white group-hover:text-[#4fc487] transition-colors">Browse Creative Library</p>
              ...
              {savedCampaigns.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2"><Bookmark className="w-5 h-5 text-[#4fc487]" /> Saved Campaigns</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-white flex items-center gap-2"><Heart className="w-4 h-4 text-[#4fc487]" /> Following</h3>
                ...
                {followedAgencies.length > 0 ? (
                  <div className="space-y-3">
                    {followedAgencies.slice(0, 5).map(c => (
                      <Link key={c.id} href={`/directory/${c.id}`} className="flex items-center gap-3 group">
                        <div className="w-8 h-8 bg-white/[0.06] rounded-lg flex items-center justify-center text-white/40 font-bold text-xs shrink-0">
                          {c.name[0]}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-white group-hover:text-[#4fc487] transition-colors truncate">{c.name}</p>
                  ...
                  <div className="text-center py-4">
                    <p className="text-sm text-white/30 mb-2">Not following any agencies yet</p>
                    <Link href="/directory">
                      <Button size="sm" className="bg-[#4fc487] hover:bg-[#45b078] text-white text-xs rounded-full">Discover Agencies</Button>
                    </Link>
                  </div>
                )}
              </div>
              ...
              <div className="glass-card p-6">
                ...
                <p className="text-white/40 text-sm mb-4">You are on the <span className="text-white font-medium">Free</span> plan.</p>
                <Link href="/pricing">
                  <Button className="w-full bg-[#4fc487] hover:bg-[#45b078] text-white font-medium text-sm rounded-full">
                    Upgrade to Pro
                  </Button>
                </Link>
              </div>
