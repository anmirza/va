...
export default function SettingsPage() {
  ...
  const isBuiltIn =
    activeRfiCategoryId === "cat-agency" ||
    activeRfiCategoryId === "cat-production";
  ...
  const updateRfiField = (
    id: string,
    field: keyof RfiField,
  ...
  const updateStepLabel = (
    idx: number,
    field: "label" | "shortLabel" | "subtitle" | "icon",
    value: string,
  ) => {
    setStepLabels((prev) =>
  ...
  const addSubSection = (stepIdx: number) => {
    setStepLabels((prev) =>
      prev.map((s, i) => {
        ...
        const subs = [
          ...(s.subSections ?? []),
          { key: `sub-${Date.now()}`, label: "New Sub-section" },
        ];
        return { ...s, subSections: subs };
      }),
    );
  ...
  const updateFieldProp = <K extends keyof RfiField>(
    id: string,
    key: K,
    value: RfiField[K],
  ) => {
  ...
  const deleteCustomField = (id: string) => {
    ...
    if (
      !confirm(
        `Delete field "${f?.label}"? Existing data for this field will remain in records but will no longer be editable.`,
      )
    )
      return;
    ...
    <div className="max-w-5xl space-y-8">
      <div>
        ...
        <p className="text-white/50 text-sm">
          Super Admin Only: Manage dynamic categories, RFI template, and
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
            <h2 className="font-semibold text-white flex items-center gap-2">
              <Settings2 className="w-4 h-4 text-[#0763d8]" /> Categories
              Catalog
            </h2>
            <button
          ...
          <div className="p-6 space-y-4">
            {categories.map((cat) => (
              <div
                key={cat.id}
                className="flex gap-4 items-start bg-white/[0.02] border border-white/[0.06] p-4 rounded-xl"
              >
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: typeof cat.iconSvg === "string" ? cat.iconSvg : "",
                  }}
                />
                <div className="flex-1 space-y-3">
                  <div>
                    <label className="text-[10px] uppercase font-bold tracking-wider text-white/40 mb-1 block">
                      Category Name
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      value={cat.iconSvg}
                      onChange={(e) =>
                        updateCategory(cat.id, "iconSvg", e.target.value)
                      }
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  <button
                    ...
                    className="bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 p-2 rounded-lg transition-colors"
                    title="Save"
                  >
        ...
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
            <h2 className="font-semibold text-white flex items-center gap-2">
              <FileText className="w-4 h-4 text-purple-500" /> RFI Structure
              Modeler
            </h2>
              ...
              {!isBuiltIn && (
                <>
                  <span className="text-xs text-white/30">
                    {rfiFields.length} fields
                  </span>
                  <button
                    onClick={async () => {
                      if (
                        confirm(
                          "Reset this RFI to empty? All custom changes will be lost.",
                        )
                      ) {
          ...
          <div className="p-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-4 mb-2 scrollbar-hide">
              {/* Built-in tabs */}
              {[
                { id: "cat-agency", name: "Agency (built-in)" },
                { id: "cat-production", name: "Production (built-in)" },
              ].map((cat) => (
                <button
                  key={cat.id}
              ...
              {categories
                .filter(
                  (c) => c.id !== "cat-agency" && c.id !== "cat-production",
                )
                .map((cat) => (
                  <button
                    key={cat.id}
            ...
            {isBuiltIn ? (
              <div>
                ...
                <div className="space-y-3">
                  {stepLabels.map((step, idx) => (
                    <details
                      key={step.key}
                      className="group/step bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden"
                    >
                      <summary className="flex items-center gap-4 p-3 cursor-pointer list-none hover:bg-white/[0.03] transition-colors">
                        <span className="w-6 h-6 rounded-full bg-[#0763d8]/20 text-[#0763d8] text-xs font-bold flex items-center justify-center shrink-0">
                          {idx + 1}
                        </span>
                        <span className="text-xs text-white/30 w-32 shrink-0 font-mono hidden sm:block">
                          {step.key}
                        </span>
                        ...
                        <span className="text-[10px] text-white/20 group-open/step:hidden">
                          ▼ expand
                        </span>
                        <span className="text-[10px] text-white/20 hidden group-open/step:inline">
                          ▲ collapse
                        </span>
                      </summary>
                      ...
                      <div className="px-4 pb-4 pt-2 border-t border-white/[0.04] space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-[1fr_160px_80px] gap-3">
                          <div>
                            ...
                            <input
                              value={step.label}
                              onChange={(e) =>
                                updateStepLabel(idx, "label", e.target.value)
                              }
                          ...
                          <div>
                            ...
                            <input
                              value={step.icon ?? ""}
                              onChange={(e) =>
                                updateStepLabel(idx, "icon", e.target.value)
                              }
                              placeholder="🏢"
                        ...
                        <div>
                          ...
                          <input
                            value={step.subtitle ?? ""}
                            onChange={(e) =>
                              updateStepLabel(idx, "subtitle", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          {(step.subSections ?? []).length === 0 && (
                            <p className="text-xs text-white/20 italic">
                              No sub-sections. Click Add to create one.
                            </p>
                          )}
                          <div className="space-y-1.5">
                            {(step.subSections ?? []).map((sec, secIdx) => (
                              ...
                              >
                                <span className="text-[10px] font-mono text-white/20 w-16 shrink-0 truncate">
                                  {sec.key}
                                </span>
                                <input
                                  value={sec.label}
                  ...
                  {stepLabels.length === 0 && (
                    <div className="text-center py-8 text-white/30 text-sm">
                      <p>
                        No step labels loaded. Labels will be seeded
                        automatically.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-[11px] text-white/30 mt-0.5">
                        Add, edit, hide, or delete the fields shown to users
                        inside each step. System fields (🔒) cannot be deleted
                        but can be hidden or relabelled.
                      </p>
                    </div>
                    <button
                      onClick={handleSaveRfi}
                  ...
                  <div className="space-y-3">
                    {stepLabels.map((step) => {
                      ...
                      return (
                        <details
                          key={step.key}
                          className="group/fstep bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden"
                        >
                          <summary className="flex items-center gap-3 p-3 cursor-pointer list-none hover:bg-white/[0.03] transition-colors">
                            <span className="text-xs text-white/40 w-32 shrink-0 font-mono hidden sm:block">
                              {step.key}
                            </span>
                            <span className="flex-1 text-sm text-white font-medium truncate">
                              {step.label}
                            ...
                            <span className="text-[10px] text-white/20 group-open/fstep:hidden">
                              ▼
                            </span>
                            <span className="text-[10px] text-white/20 hidden group-open/fstep:inline">
                              ▲
                            </span>
                          </summary>
                          ...
                          <div className="px-4 pb-4 pt-2 border-t border-white/[0.04] space-y-2">
                            {stepFields.length === 0 && (
                              <p className="text-xs text-white/30 italic py-2">
                                No fields configured for this step yet.
                              </p>
                            )}
                            {stepFields.map((f) => (
                              <div
                                key={f.id}
                                className="flex flex-wrap items-start gap-2 bg-white/[0.02] border border-white/[0.05] rounded-lg p-2.5"
                              >
                                {/* Reorder */}
                                <div className="flex flex-col shrink-0">
                                  <button
                                    onClick={() =>
                                      moveFieldWithinStep(f.id, -1)
                                    }
                                    className="text-white/30 hover:text-white text-xs leading-none"
                                  >
                                    ▲
                                  </button>
                                  <button
                                    onClick={() => moveFieldWithinStep(f.id, 1)}
                                    className="text-white/30 hover:text-white text-xs leading-none"
                                  >
                                    ▼
                                  </button>
                                ...
                                {/* Lock indicator */}
                                <div
                                  className="shrink-0 pt-1.5 w-5 text-center"
                                  title={
                                    f.isSystem
                                ...
                                <div className="flex-1 min-w-[180px]">
                                  <label className="text-[9px] uppercase tracking-wider text-white/30 block">
                                    Label
                                  </label>
                                  <input
                                    value={f.label}
                                  ...
                                  <select
                                    ...
                                    onChange={(e) =>
                                      updateFieldProp(
                                        f.id,
                                        "type",
                                        e.target.value as RfiField["type"],
                                      )
                                    }
                                    disabled={f.isSystem}
                            ...
                            >
                              <Plus className="w-3 h-3" /> Add Field to{" "}
                              {step.label}
                            </button>
                          </div>
                        </details>
              ...
              <div>
                {rfiFields.map((f, idx) => (
                  <div
                    key={f.id}
                    className="flex gap-3 items-start bg-white/[0.02] border border-white/[0.06] p-4 rounded-xl group hover:border-white/[0.12] transition-colors"
                  >
                    {/* Reorder handle */}
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-[1fr_140px_120px] gap-3">
                      <div>
                        ...
                        <input
                          value={f.label}
                          onChange={(e) =>
                            updateRfiField(f.id, "label", e.target.value)
                          }
                      ...
                      <div>
                        ...
                        <select
                          value={f.type}
                          onChange={(e) =>
                            updateRfiField(f.id, "type", e.target.value)
                          }
                      ...
                      <div>
                        ...
                        <input
                          value={f.section || ""}
                          onChange={(e) =>
                            updateRfiField(f.id, "section", e.target.value)
                          }
                          placeholder="General"
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <label className="flex items-center gap-1.5 cursor-pointer select-none">
                          ...
                          <input
                            ...
                            checked={f.required}
                            onChange={(e) =>
                              updateRfiField(f.id, "required", e.target.checked)
                            }
                ...
                {rfiFields.length === 0 && (
                  <div className="text-center py-8 text-white/30 text-sm">
                    ...
                    <p className="text-xs text-white/20 mt-1">
                      Add fields below to build your RFI template.
                    </p>
                  </div>
                )}
                <button
