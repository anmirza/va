...
export default function GenericListingPage() {
  ...
  const load = useCallback(async () => {
    const [allOrgs, cats] = await Promise.all([
      getAllOrgsFS(),
      getVACategoriesFS(),
    ]);
  ...
  const editHref = (id: string) =>
    categoryId === "cat-agency"
      ? `/admin/agencies/create?edit=${id}`
      : categoryId === "cat-production"
        ? `/admin/production/create?edit=${id}`
        ...
        <AlertDialogContent className="bg-[#0a0b1a] border border-white/10">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">
              {removeTarget?.mode === "hard" ? "Permanently Delete" : "Remove"}{" "}
              {categoryName}
            </AlertDialogTitle>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-3">
            {categoryName}
            <span className="text-sm font-normal text-white/30 bg-white/5 px-2 py-0.5 rounded-full">
              {filteredOrgs.length}
            </span>
          </h1>
          <p className="text-sm text-white/50 mt-1">
            Manage and monitor all registered {categoryName.toLowerCase()}.
          </p>
        </div>
        <Button
        ...
        <div className="glass-card rounded-2xl p-16 text-center">
          ...
          <p className="text-white/40 text-sm">
            {searchQuery
              ...
              : `No ${categoryName.toLowerCase()} registered yet.`}
          </p>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Link href={editHref(org.id)}>
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-8 border-white/[0.1] text-white/60 hover:text-white rounded-lg text-xs gap-1.5"
                  >
