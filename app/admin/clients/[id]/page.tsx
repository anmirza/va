...
import {
  getClientCompanyByIdFS,
  getClientUsersByCompanyFS,
  createClientUserFS,
  updateClientCompanyFS,
  addClientCompanyTokensFS,
  suspendClientUserFS,
...
export default function ClientCompanyDetailPage() {
  ...
  const handleAddTokens = async () => {
    ...
    toast.success(
      `Added ${addTokensAmount} package${addTokensAmount > 1 ? "s" : ""} (${addTokensAmount * packageSize} agency slots)`,
    );
    load();
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      const created = await createClientUserFS(
      ...
      toast.success(
        `User "${created.name}" created. They will set their password on first login.`,
      );
    } catch {
  ...
  if (!company) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <Link
          href="/admin/clients"
          className="text-[#0763d8] text-sm hover:underline"
        >
  ...
  const inputCls =
    "w-full h-9 bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-[#0763d8]/60";
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="mb-6">
        <Link
          href="/admin/clients"
          className="inline-flex items-center gap-2 text-sm text-white/40 hover:text-white mb-5 transition-colors"
        >
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 text-xs text-white/40 ml-13">
              {company.holdingCompany && (
                <>
                  <span>{company.holdingCompany}</span>
                  <ChevronRight className="w-3 h-3" />
                </>
              )}
              {company.regionalHub && (
                <>
                  <span>{company.regionalHub}</span>
                  <ChevronRight className="w-3 h-3" />
                </>
              )}
              {company.region && (
          ...
          <span
            className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${company.status === "active" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-red-500/10 border-red-500/20 text-red-400"}`}
          >
            {company.status}
          </span>
        </div>
        ...
        <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-red-300 font-semibold">
              All Agency Search &amp; Selection credits exhausted
            </p>
            <p className="text-xs text-red-300/70 mt-1 italic">
              "Contact VA Consulting to extend your programme." — Add more
              packages below to restore access.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="glass-card rounded-2xl overflow-hidden">
            ...
            {showNewUser && (
              <form
                ...
                className="p-6 border-b border-white/[0.06] bg-white/[0.02]"
              >
                <div className="bg-amber-500/5 border border-amber-500/15 rounded-xl p-3 mb-4 flex gap-2">
                  <Shield className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                  <p className="text-xs text-amber-300/80 leading-relaxed">
                    Company will be auto-set to{" "}
                    <strong className="text-amber-300">{company.name}</strong>.
                    On first login, the user sets their password and must
                    complete Region, Country, Role, and Mobile in Account
                    Information.
                  </p>
                </div>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-white text-black font-semibold rounded-lg text-sm hover:bg-white/90 transition-colors disabled:opacity-60 flex items-center gap-2"
                  >
                    {isCreatingUser ? (
                      <>
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {users.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3.5">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-semibold ${u.status === "active" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
                      >
                        {u.status}
                      </span>
                    </td>
                    <td className="px-6 py-3.5 text-right">
                      {u.status === "active" && (
                        <button
                          ...
                          className="text-xs text-white/30 hover:text-red-400 transition-colors flex items-center gap-1 ml-auto"
                        >
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div
                className={`rounded-xl p-3 text-center ${isExhausted ? "bg-red-500/10" : "bg-emerald-500/10"}`}
              >
                <p
                  className={`text-2xl font-bold ${isExhausted ? "text-red-400" : "text-emerald-400"}`}
                >
                  {tokensRemaining}
                </p>
                <p
                  className={`text-[10px] uppercase tracking-wider mt-0.5 ${isExhausted ? "text-red-400/60" : "text-emerald-400/60"}`}
                >
                  Remaining
                </p>
              </div>
            ...
            <p className="text-xs text-white/30 mb-3">
              ...
              <span className="text-white/60">
                {company.packageSize || 6} agencies/package
              </span>
            </p>
          ...
          <div className="glass-card rounded-2xl p-5">
            ...
            <div className="space-y-2.5 text-xs">
              {[
                {
                  label: "Holding",
                  value: company.holdingCompany || company.holding,
                },
                {
                  label: "Operate As",
                  value: company.operateAs
                    ? company.operateAs
                        .replace("_", " ")
                ...
                d.value ? (
                  ...
                  >
                    ...
                    <span className="text-white/70 text-right max-w-[160px] truncate">
                      {d.value}
                    </span>
                  </div>
                ) : null,
            ...
            {company.notes && (
              <div className="mt-3 pt-3 border-t border-white/[0.05]">
                <p className="text-[10px] uppercase text-white/30 tracking-wider mb-1">
                  Notes
                </p>
          ...
          <div className="glass-card rounded-2xl p-5">
            ...
            <p className="text-xs text-white/40 leading-relaxed">
              ...
              <strong className="text-white/60">{company.name}</strong> are
              scoped exclusively to this entity.
              {company.holdingCompany &&
                company.holdingCompany !== company.name && (
                  <>
                    {" "}
