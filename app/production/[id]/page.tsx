      ...
      <main className="flex-1">
        <div className="relative h-64 overflow-hidden">
          ...
          <div className="absolute top-4 left-4 sm:left-8">
            <Link href="/production" className="flex items-center gap-1.5 text-sm text-white/80 hover:text-white bg-black/30 backdrop-blur px-3 py-1.5 rounded-lg">
              <ArrowLeft className="w-4 h-4" /> Production Companies
            </Link>
          </div>
        </div>

        <div className="bg-[#2e3843] px-4 sm:px-6 lg:px-8 pb-10 -mt-1">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <p className="text-[#666] leading-relaxed">{company.about}</p>
              </div>
              <div className="bg-white rounded-xl p-6 shadow-sm">
                <h2 className="font-bold text-[#1a1a1a] mb-4">Specialties</h2>
                <div className="flex flex-wrap gap-2">
                  {company.specialties.map(s => (
                    <span key={s} className="px-3 py-2 bg-[#eef0f3] rounded-lg text-sm font-medium text-[#2e3843]">{s}</span>
                  ))}
                </div>
              </div>
              {company.clients.length > 0 && (
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  <h2 className="font-bold text-[#1a1a1a] mb-4">Selected Clients</h2>
                  <div className="flex flex-wrap gap-2">
                    {company.clients.map(c => (
                      <span key={c} className="px-3 py-1.5 border border-[#d8dce2] rounded-full text-sm text-[#666]">{c}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-xl p-6 shadow-sm">
                <h3 className="font-bold text-[#1a1a1a] mb-4">Details</h3>
                <dl className="space-y-3 text-sm">
                  <div className="flex justify-between"><dt className="text-[#666]">City</dt><dd className="font-medium">{company.city}</dd></div>
                  <div className="flex justify-between"><dt className="text-[#666]">Country</dt><dd className="font-medium">{company.country}</dd></div>
                  <div className="flex justify-between"><dt className="text-[#666]">Founded</dt><dd className="font-medium">{company.founded}</dd></div>
                  <div className="flex justify-between"><dt className="text-[#666]">Team Size</dt><dd className="font-medium">{company.employees}</dd></div>
                </dl>
                {company.website && (
