      ...
      <main className="flex-1">
        <div className="relative h-72 overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 px-4 sm:px-6 lg:px-8 pb-8 max-w-4xl mx-auto">
            <span className="inline-block px-3 py-1 bg-[#4fc487]/80 text-white text-xs font-medium rounded-full mb-3">{interview.category}</span>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <article className="lg:col-span-2">
              <Link href="/interviews" className="flex items-center gap-1.5 text-sm text-[#666] hover:text-[#4fc487] mb-6 transition-colors">
              ...
              <div className="bg-white rounded-xl p-6 sm:p-8 shadow-sm">
                <p className="text-lg text-[#444] mb-6 leading-relaxed font-medium italic border-l-4 border-[#4fc487] pl-4">{interview.excerpt}</p>
                <div className="space-y-4 text-[#444] leading-relaxed">
                  {interview.content.split('\n\n').map((para, i) => {
                    if (para.startsWith('**') && !para.startsWith('**A') && !para.startsWith('**T') && !para.startsWith('**W') && !para.startsWith('**H')) {
                      return <h3 key={i} className="font-bold text-[#1a1a1a] text-base mt-6 text-[#4fc487]">{para.replace(/\*\*/g, '')}</h3>
                    }
